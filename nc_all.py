...
def extract_tiff_data(tiff_file, df, lat_range=5.0, lon_range=5.0):
    ...
    return tiff_column
tiff_files = [os.path.join(tif_folder, f) for f in os.listdir(tif_folder) if f.endswith('.tif')]

tiff_progress_bar = tqdm(tiff_files, desc="Processing TIFF files", unit="file", leave=True)

for tiff_file in tiff_progress_bar:
    ...
    tiff_progress_bar.set_postfix({"Files processed": f"{tiff_progress_bar.n}/{len(tiff_files)}"})



output_file = 'merged_all.xlsx'
